...
export const generateQuiz = async ({ text, apiKey, numQuestions, difficulty }: GenerateQuizParams): Promise<QuizQuestion[]> => {
...
`;
    ...
    let raw = candidates[0].content.parts.map((p: any) => p.text).join('');
