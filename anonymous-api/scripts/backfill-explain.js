#!/usr/bin/env node
/*
  Simple Node backfill script (JS) that connects to MongoDB using MONGODB_URI env var
  and updates Question documents for a given quizId by matching question text and
  setting the explain field.

  Usage:
    export MONGODB_URI="mongodb://..."
    export DATABASE_NAME="your_db"
    node scripts/backfill-explain.js

  This script does NOT depend on TypeScript path aliases.
*/
const mongoose = require('mongoose')

const MONGODB_URI = process.env.MONGODB_URI
const DATABASE_NAME = process.env.DATABASE_NAME || undefined
...
async function run() {
  ...
  const qid = mongoose.Types.ObjectId(quizId)

  for (const item of items) {
    const res = await Question.updateOne({ quizId: qid, question: item.question }, { $set: { explain: item.explain } })
